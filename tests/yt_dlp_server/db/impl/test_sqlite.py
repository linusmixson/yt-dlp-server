...
class TestSQLiteDBConnection:
    ...
    def test_create_tables(self, db):
        ...
        cursor = db.connection.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='task'")
...
class TestSQLiteDBTaskOperations:
    ...
    def test_get_task_nonexistent(self, db):
        ...
        nonexistent_task = Task(job_id="nonexistent", url="https://example.com/none.mp4")
